...
public class FtpUtils {
    ...
    public static void main(String[] args) {
        FtpUtils f = new FtpUtils("139.199.248.109", 21, "apsftp", "apsftp");
        try {
            if(f.open()) {
        ...
        if (ftpClient != null) {
            ...
            try {
                ...
                if(flag){
                    ...
                    System.out.println("上传文件失败，请查找原因");
                }
            } catch (Exception e) {
                this.close();
                e.printStackTrace();
     ...
     * @return
     */
    public boolean upload(FileInputStream fileInputStream, String ftpFileName, String ftpDirectory) {
        if (!ftpClient.isConnected()) {
            return false;
        }
        boolean flag = false;
        if (ftpClient != null) {
            try {
                ...
                ftpClient.setFileType(FTPClient.BINARY_FILE_TYPE);
                // 上传
                flag = ftpClient.storeFile(new String(ftpFileName.getBytes(), "iso-8859-1"), fileInputStream);
            } catch (Exception e) {
                this.close();
                e.printStackTrace();
                return false;
        ...
        System.out.println("上传文件成功，上传到目录：" + ftpDirectory + "/" + ftpFileName);
        return flag;
    }
     ...
     * @return
     */
    public FileOutputStream download(String ftpDirectoryAndFileName, FileOutputStream fileOutputStream) {
        if (!ftpClient.isConnected()) {
            return null;
        }
        ftpClient.enterLocalPassiveMode(); // Use passive mode as default
        try {
            ...
            // file
            logger.info(ftpClient.getReplyString()); // check result
            logger.info("从ftp服务器上下载文件：" + ftpDirectoryAndFileName + "， 保存到文件流" );
